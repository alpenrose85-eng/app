...
for i in range(num_tests):
    ...
    with col2:
        sigma = col2.number_input(f"σ, МПа (исп. {i+1})", value=120.0, min_value=0.1, max_value=500.0)
    with col3:
        T_C = col3.number_input(f"T, °C (исп. {i+1})", value=600.0, min_value=100.0, max_value=1000.0)
    with col4:
        tau_h = col4.number_input(f"τ, ч (исп. {i+1})", value=500.0, min_value=1.0, max_value=1e7)
...
with col1:
    s_nom = st.number_input("Номинальная толщина стенки s_н, мм", value=6.0, min_value=0.1, max_value=1000.0)
    s_min = st.number_input("Текущая min толщина s_мин, мм", value=5.07, min_value=0.1, max_value=s_nom)
    s_max = st.number_input("Текущая max толщина s_макс, мм", value=5.95, min_value=0.1, max_value=1000.0)
    tau_exp = st.number_input("Наработка τ_э, ч", value=317259, min_value=1, max_value=5_000_000)
with col2:
    d_max = st.number_input("Макс. внутр. диаметр d_макс, мм", value=19.90, min_value=0.1, max_value=1000.0)
    T_rab_C = st.number_input("Рабочая температура T_раб, °C", value=517.0, min_value=100.0, max_value=1000.0)
    p_MPa = st.number_input("Давление пара p, МПа", value=27.93, min_value=0.1, max_value=100.0)
    k_zapas = st.number_input("Коэффициент запаса k_зап", value=1.5, min_value=1.0, max_value=5.0)
